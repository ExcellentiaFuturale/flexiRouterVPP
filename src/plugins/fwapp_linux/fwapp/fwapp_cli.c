...
unformat_fwapp_interface (unformat_input_t * input, va_list * args)
{
  ...
  if (cfg->type == FWAPP_INTERFACE_UNDEFINED) {
    clib_error ("interface type was not provided");
    return 0;
  }
  return 1;
}
...
static clib_error_t *
fwapp_add_del_app_cmd (vlib_main_t * vm, unformat_input_t * input, vlib_cli_command_t * cmd)
{
  u32                     is_add = 0;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "del %s", &cfg.name))
        ;
      else if (unformat (input, "add %s", &cfg.name))
        is_add = 1;
  ...
  if (!cfg.name)
    {
      ret = (clib_error_return (0, "no app name was provided"));
      goto done;
    }
  ...
  if (app && is_add)
  {
      ret = (clib_error_return (0, "app %s exists", cfg.name));
      goto done;
  }
  else if (app==NULL && is_add==0)
  {
      ret = (clib_error_return (0, "app %s was not found", cfg.name));
      goto done;
  }
  ...
  if (is_add)
    ...
    res = fwapp_del_app (app);
  if (res)
  {
      ret = clib_error_return (0, "failed to %s app %s: %u",
                               (is_add?"add":"delete"), cfg.name, res);
      goto done;
  }
...
VLIB_CLI_COMMAND (fwapp_app_add_del_command, static) = {
  .path = "fwapp app",
  .function = fwapp_add_del_app_cmd,
  .short_help = "fwapp app [add|del] <name> [descr '<value>'] <monitor|divert> [fallback <drop>] "
                "[next <name>] [acl <index>] interface app <tap|dpdk> <vpp_if_name>|<sw_if_index> [src <vpp_if_name>|<sw_if_index>] "
                "[interface ...] "
};
/* *INDENT-ON* */
...
static clib_error_t *
fwapp_attach_detach_app_cmd (vlib_main_t * vm, unformat_input_t * input, vlib_cli_command_t * cmd)
{
  u32                     is_attach = 0;
  clib_error_t*           ret = NULL;
  u32                     res;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (input, "attach %s", &app_name))
        is_attach = 1;
      else if (unformat (input, "detach %s", &app_name))
        ;
 ...
 * Attach an ABF policy to an interface.
 */
VLIB_CLI_COMMAND (fwapp_attach_detach_app_command, static) = {
  .path = "fwapp app",
  .function = fwapp_attach_detach_app_cmd,
  .short_help = "fwapp app [attach|detach] <name> [app <tap|dpdk> <vpp_if_name>|<sw_if_index>] src <vpp_if_name>|<sw_if_index> "
                "[[app <tap|dpdk> <vpp_if_name>|<sw_if_index>] src <vpp_if_name>|<sw_if_index>  ...] "
};
/* *INDENT-ON* */
